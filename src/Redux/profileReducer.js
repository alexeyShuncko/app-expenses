...
let token =
  'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjoxNjc4MjY4NzU1LCJqdGkiOiJmNjI1ZDRiNTRjMWI0ZDk0OTg0ZDM5YzQ4OGE1NmJlNSIsInVzZXJfaWQiOjV9.KmQC4edEQn7KP5yPOYJ4estTNgc4gGM6Y3w5_QFTPe0';
  ...
  return new Promise((resolve, reject) => {
    createUser(name, password, email)
      ...
      .then(() => {
        ...
        postSalary(1, 13, 1).then(() => postSalary(1, 13, 3));
      })
      .catch(() => reject());
  });
};
  ...
  if (name === 'testUser') {
    return new Promise(() => {
      ...
      postDataUser(localStorage.getItem('key'))
        ...
        .catch(() =>
          createToken(name, password).then((data) => {
            token = data.access;
    ...
    return new Promise((resolve, reject) => {
      createToken(name, password)
        ...
        .then(() => {
          postDataUser(localStorage.getItem('key')).then(() =>
