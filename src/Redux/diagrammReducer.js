...
let initialState = {
  category: [
    ...
    {
      name: 'Алкоголь',
      nameRusСase: 'Алкоголь',
      color: 'rgb(33, 33, 209)',
      id: 2,
      data: [{ id: 20001, created: '2022-04-08', amount: 40 }],
      summ: 40,
    },
    {
      name: 'Квартира',
      nameRusСase: 'Квартиру',
      color: 'rgb(87, 217, 255)',
      id: 3,
      data: [{ id: 30001, created: '2022-04-11', amount: 15 }],
      summ: 15,
    },
    {
      name: 'Транспорт',
      nameRusСase: 'Транспорт',
      color: 'rgb(22, 153, 40)',
      id: 4,
      data: [{ id: 40001, created: '2022-04-09', amount: 25 }],
      summ: 25,
    },
  ],
  activ: '',
...
const diagrammReduser = (state = initialState, action) => {
  switch (action.type) {
    ...
    case ADD_SELECT_DIAGRAMM:
      return {
        ...state,
        selectDiagramm: action.selectDiagramm,
      };
    ...
    case ADD_SELECT_DIAGRAMM_STAT:
      return {
        ...state,
        selectDiagrammStat: action.selectDiagrammStat,
      };
...
export const addIncome = (created, amount, category) => (dispatch) => {
  postIncomes(created, amount, category).then(() => dispatch(sources()));
};
// Расходы
...
export const addDiagramm = (data) => (dispatch) => {
  postExpenses(data).then(() => dispatch(categories()));
};
...
export const deleteItemCategories = (id) => (dispatch) => {
  deleteСategories(id).then(() => dispatch(categories()));
};

// Переименование категории
export const updateItemCategories =
  (name, color, id, activID) => (dispatch) => {
    getItem(name)
...
export const updateColor = (name, nameRusСase, color, id) => (dispatch) => {
  putСategories(name, nameRusСase, color, id).then(() =>
    dispatch(categories())
  );
};
...
export const nameCaseRelativity = (name, unit, prise) => (dispatch) => {
  getItem(name)
    .then((data) =>
      postRelativity(data.Р, unit, prise, {
        name1: data.Р,
        name2: data.множественное ? data.множественное.Р : data.Р,
        name3: name,
      })
    )
...
export const getValute = () => (dispatch) => {
  return new Promise((resolve, reject) => {
    Promise.all([getDollar(), getEuro(), getRuble()]).then((values) => {
...
export const deleteAppRecording = (category, id) => (dispatch) => {
  return new Promise((resolve, reject) => {
    deleteRecording(category, id).then((data) => {
