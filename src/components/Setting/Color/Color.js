...
const Color = (props) => {
  ...
  const onColor = (value) => {
    form.setFieldsValue({
      color: Converter_V_HEX(
        props.diagramm.category.find((a) => a.name === value).color
      ),
    });
  ...
  const validator = (rule, value) => {
    if (
      props.diagramm.category.find((a) => a.color === Converter_V_RGB(value))
    ) {
    ...
    <div className={s.changeСolor}>
      ...
      <div className={s.color}>
        <Form
          ...
          onFinish={onFinish}
          autoComplete="off">
          <Form.Item label="Категория" name="name" style={{ marginBottom: 10 }}>
            <Select
        ...
        <div className={s.instruction}>
          <div>
            <div>
              1) В поле "Название категории" выберите из выпадающего списка
