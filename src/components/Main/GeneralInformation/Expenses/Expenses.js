...
const Expenses = (props) => {
  ...
  const onFinish = (values) => {
    ...
    for (let key in values) {
      if (values[key] && !arrSuffics.includes(key)) {
        ...
        valutaArray.push(
          values[
            arrSuffics[diagramm.indexOf(diagramm.find((a) => a.name === key))]
          ]
        );
      }
    }
    ...
    if (keyArray.length === 0) {
      ...
      const converterValute = (arr, index) => {
        if (valutaArray[index] === 'BYN') {
          ...
          return (
            arr[index] * props.diagramm.exchangeRates.dollar.Cur_OfficialRate
          ).toFixed(2);
        } else if (valutaArray[index] === 'EUR') {
          return (
            arr[index] * props.diagramm.exchangeRates.euro.Cur_OfficialRate
          ).toFixed(2);
      ...
      let text = keyArray
        .map((a) => diagramm.find((b) => b.name === a).nameRusCase)
        .join(', ');
  ...
  const onMouseOver = (e) => {
    e.target.style.borderColor = diagramm.find(
  ...
  const suffixSelector = diagramm.map((a, index) => (
    ...
    <div>
      {!editMode ? (
        <div className={s.buttonExpenses}>
          <Button type="primary" size="large" danger onClick={activateEditMode}>
            Добавить расходы
          </Button>
        </div>
      ) : (
