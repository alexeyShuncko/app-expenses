...
const Income = (props) => {
  ...
  const onFinish = (values) => {
    ...
    let category = props.diagramm.income.data.filter(
    ...
    <div>
      {!editMode ? (
        <div className={s.incomeButton}>
          <Button type="primary" size="large" onClick={activateEditMode}>
            Добавить доходы
          </Button>
        </div>
      ) : (
        <div className={s.formIncomeFild}>
          ...
          <div>
            <Form
              ...
              autoComplete="off">
              <Form.Item label="Доход" name="name" style={{ marginBottom: 0 }}>
                <Select>
                  {props.data.map((a) => (
                    <Select.Option
