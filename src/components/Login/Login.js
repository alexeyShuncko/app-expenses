...
const Login = ({ getUser, ...props }) => {
  ...
  const onFinish = (values) => {
    if (
      props.profile.users &&
      !props.profile.users.map((a) => a.username).includes(values.username)
    ) {
      form.setFields([
        {
      ...
      values.password !== props.profile.test.password
    ) {
      ...
      if (values.remember) {
        ...
        localStorage.setItem(
          'user',
          JSON.stringify(
            props.profile.users.find((a) => a.username === values.username)
          )
        );
      }
      props.profile.users &&
  ...
  return (
    <div className={s.container}>
      <div
        ...
        <div className={s.passLog}>
          Логин:
          <span id="LoginCopy" className={s.testData}>
            testUser
          </span>
          <span className={s.popup} id="copyLogin">
            Копировать и вставить
          </span>
          <span className={s.popupGreen} id="copyLoginSuccses">
            Успешно скопировано
          </span>
        </div>
        <div className={s.passLog}>
          Пароль:
          <span className={s.testData} id="PasswordCopy">
            {' '}
            test1234Q
          </span>
          <span className={s.popup} id="copyPass">
            Копировать и вставить
          </span>
          <span className={s.popupGreen} id="copyPassSuccses">
            Успешно скопировано
          </span>
        </div>
      </div>
...
export default connect(mapStateToProps, { addActionUser, getUser, login })(
  Login
);
