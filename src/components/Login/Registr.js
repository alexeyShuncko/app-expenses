...
const Registr = (props) => {
  ...
  const onFinish = (values) => {
    props
      .registration(values.username, values.password, values.email)
      .catch(() =>
        form.setFields([
          { errors: [`Слишком распространённый пароль!`], name: 'password' },
        ])
      );
  };
  ...
  const validator = (_, value) => {
    ...
    if (value && value.length < 8) {
      return Promise.reject(
        new Error('Пароль слишком короткий, минимум 8 символов!')
      );
    } else if (!reg.test(value)) {
      return Promise.reject(
        new Error('Пароль не должен состоять только из цифр!')
      );
