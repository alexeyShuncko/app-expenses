...
const DiagrammIncome = (props) => {
  ...
  const diagramm =
    props.diagrammSelect === 'доходов'
      ? props.income.map((a) => {
          return {
            ...a,
            data: a.data.filter(
              (a) =>
                a.created <= (props.periodDiagramm.Po || props.todayPo) &&
  ...
  const totalDiag = diagramm
    .map((a) =>
      a.data.map((e) => e.amount).reduce((sum, current) => sum + current, 0)
    )
  ...
  let textMessage = `У Вас нет ${props.diagrammSelect} ...`;
  // с ${dateS} по ${datePo}
      ...
      return diagramm.map((a) => {
        return {
          ...
          value: (
            a.data
              .map((e) => e.amount)
              .reduce((sum, current) => sum + current, 0) / props.dollar
          ).toFixed(0),
        };
      });
      ...
      return diagramm.map((a) => {
        return {
          ...
          value: (
            a.data
              .map((e) => e.amount)
              .reduce((sum, current) => sum + current, 0) / props.euro
          ).toFixed(0),
        };
      });
